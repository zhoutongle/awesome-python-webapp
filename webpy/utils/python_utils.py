...
PNG_PATH = "\\".join(currpath.split("\\")[:-2]) + "\\data\\3.png"
CNWORD_PATH = "\\".join(currpath.split("\\")[:-2]) + "\\data\\cnword.txt"
STOPWORDS_PATH = "\\".join(currpath.split("\\")[:-2]) + "\\data\\stopwords.txt"
PNG_1_PATH = "\\".join(currpath.split("\\")[:-2]) + "\\data\\20th.png"

back_color = imread(PNG_PATH)  # 解析该图片
