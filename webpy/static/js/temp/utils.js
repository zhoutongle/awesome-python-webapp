...
function custConfirm(title,message,buttons){
    var confdialog = $('<div id="confdialog" title="'+title+'" class="ui-state-error ui-corner-all" style="line-height:25px;"><p><span class="info-icon ui-icon ui-icon-alert"></span><strong>警告: </strong> '+message+'</p></div>').appendTo('body');
    var defbuttons = {
        Cancel: function() {
                        $(this).dialog('close');
                },
        ...
        $.fn.ipaddress = function(params) {
                return $(this).each(function(){
                        ...
                        $('input.ip_octet.' + $this.attr('id')).bind('keydown', function(e){
                                ...
                                var prev_octet = $(this).prevUntil('input.ip_octet' + $this.attr('id'));
...
function checkSession(){
    ...
    $.ajax({
        ...
        success: function(text){
            if(!isNaN(text)){
                ...
                custAlert("操作警告","登录超时，请重新登录", function(){ location.replace('https://' + location.host + '/login?redirect_url=' + location.pathname);});
            }
        },
        error: function(){
                location.host.replace(':81','');
    ...
    function getprocessing(){
        ...
        $.ajax({
            ...
            success: function(text){
                setCookie('comet_flag_2', '0', 1);
                if($('#loaddialog')){
                    $('#loaddialog p.loadword').html(text);
                }
                else{
                    custLoading(text);
                }
                ...
                success: function(text){
                    ...
                    for(var i = 2;i >=0 ;i--){
                        ...
                        default:
                        {
                                ...
                                nodepanel += "</table>";
                                //$("#upspanel").before(nodepanel);
                                nodepanel='<div id="mgroup"><div id="mtop"></div>'+nodepanel+'</div><div id="mbottom"></div></div>';
                                if($(obj).find('#mgroup').length > 0){
                                        if(diskname != undefined){
                                                $("#"+nodename+"_"+diskname).addClass('highlight')
                                        }
                                        return false;
                                }else{
                                        if(obj != undefined){
                                                $(obj).append(nodepanel);
                                        }else{
                                                return nodepanel;
                                        }
                                }
                            ...
                            /*$("#" + nodename).bind('mouseenter', function(){
                                $("#machineinfo").html("");
                                $("#machineinfo").show(getraidinfo(nodename));
                                $("#" + nodename).append($("#machineinfo"));
                                var nodetop = parseInt($("#" + nodename).position().top);
                                $("#machineinfo").addClass("machineinfo");
                                $("#machineinfo").attr("style","top:" + nodetop + "px;");
                                $("#machineinfo").addClass('ui-corner-all');
                            });
                            $("#" + nodename).bind('click', function(){
                                window.location.href=$(this).attr('href').toString();
                                });
                            $("#" + nodename).bind('mouseleave', function(){
                                $("#machineinfo").hide();
                            });*/
                }
        });
    ...
    function init_navleft(){
            ...
            var zNodes = [{'name':'存储组',isParent:"true", 'fullName':'存储组'},{'name':'存储服务管理',isParent:"true", 'fullName':'存储服务管理'}];
